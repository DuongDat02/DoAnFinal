...
const Max30100Data = () => {
  ...
  const fetchData = async (page) => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/data/max30100/alldata?page=${page}&limit=20`, {
        method: 'GET',
        headers: {
  ...
  const fetchSearchData = async (page) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:4000/api/data/max30100/search?startDate=${formattedStartDate}&endDate=${formattedEndDate}&page=${page}&limit=20`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div className='sensordata-content'>
      <h1>Dữ liệu nhịp tim và nồng độ Oxy trong máu</h1>
      ...
      <table>
        ...
        <tbody>
          {data.map((item) => (
            <tr key={item.id}>
              <td>{item.heart.toFixed(2)} bpm</td>
              <td>{item.oxy} %</td>
